  ...
  Widget build(BuildContext context) {
    ...
    return PhoneScaffold(
      child: ListView(
        ...
        children: [
          const TopBar(title: 'Bookings', subtitle: 'Manage your guests and stays'),
          ...
          if (_mainTab == 0) _buildMyGuests() else _buildMyStays(bookingsProvider),
        ],
      ),
    );
  }
  ...
  Widget _buildMyGuests() {
    if (_subTab == 0) {
      return Column(children: [
        _ownerBookingCard(context, Sample.azure, 'Omar Khalil', '★ 4.9 · ID ✓ · 12 stays',
            ['Jun 21–25', '4 guests · 2A 2C', 'EGP 18,000'], 'Upcoming', BadgeKind.navy),
        const SizedBox(height: 14),
        _ownerBookingCard(context, Sample.dunes, 'Sara Mansour', '★ 4.6 · ID ✓ · 3 stays',
            ['Jul 2–6', '2 guests', 'EGP 15,200'], 'Upcoming', BadgeKind.navy, grayscale: true),
      ]);
    } else if (_subTab == 1) {
      return _ownerBookingCard(context, Sample.lagoon, 'Nour Adel', '★ 5.0 · checked in today',
          ['Jun 14–18', '4 guests', 'EGP 22,400'], 'Checked in', BadgeKind.green, active: true);
    } else {
      return _ownerBookingCard(context, Sample.dunes, 'Hana Tarek', '★ checked out Jun 11',
          ['Jun 8–11', '2 guests', 'EGP 11,400'], 'Done', BadgeKind.gray, rated: true);
    }
  }
    ...
    return GestureDetector(
      onTap: () {
        final route = switch (booking.status) {
          BookingStatus.upcoming => '/booking-upcoming',
          BookingStatus.active => '/booked-property',
          _ => '/booking-past',
        };
        context.push(route, extra: booking);
      },
      child: Container(
            ...
            Row(
              ...
              children: [
                ClipRRect(
                  ...
                  child: SahelyImage(imageUrl: booking.imageUrl, width: 84, height: 84, enableViewer: false),
                ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          StatusBadge(booking.statusText, 
                            kind: isActive ? BadgeKind.green : (isPast ? BadgeKind.gray : BadgeKind.navy)),
                        ],
                      ),
                      ...
                      Row(children: [
                        ...
                        Text(booking.location, style: AppTheme.dm(size: 12, color: const Color(0xFF5B5B5B))),
                      ]),
                      const SizedBox(height: 4),
                      Text(booking.dates, style: AppTheme.dm(size: 12, color: AppColors.navy, weight: FontWeight.w600)),
                      const SizedBox(height: 2),
                      Text('Order no. ${booking.orderNumber}', style: AppTheme.dm(size: 12, color: const Color(0xFF5B5B5B))),
                    ],
                  ),
                ),
              ],
