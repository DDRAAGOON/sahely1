...
class _OwnerHomeScreenState extends State<OwnerHomeScreen> {
  ...
  /// Pull to refresh: the dashboard, the account and the current category.
  ...
  /// "All" is the dashboard's own trending rail; every other chip is a real
  /// search, because a listing's type and distance to the beach are only
  /// known to the server.
  Future<void> _applyCategory() async {
    final category = _selectedCategory ?? 'All';
    final request = ++_categoryRequest;
    if (category == 'All') {
      setState(() => _filteredProperties =
          _dashboard?.trendingProperties.take(3).toList() ?? []);
      return;
    }
    try {
      final result = await sl<RenterRepository>()
          .searchProperties(PropertyQuery.category(category), limit: 3);
                  ...
                  children: [
                        ...
                        children: [
                          Column(
                              ...
                              children: [
                                ...
                                Text(
                                    (dashboard?.ownerName.isNotEmpty ?? false)
                                        ? dashboard!.ownerName
                        ...
                        onTap: () {
                          AppNavigation.goToFilters(
                            ...
                            onApplyFilters: (result) {
                              AppNavigation.goToSearchResults(context,
                                  extra: result);
                            },
                          );
                        },
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          ChoiceChipPill(AppLocalizations.of(context).catVilla,
                              selected: _selectedCategory == 'Villa',
                              onTap: () {
                            setState(() {
                              _selectedCategory = (_selectedCategory == 'Villa')
                                  ? 'All'
                                  : 'Villa';
                            });
                            _applyCategory();
                          ...
                          ChoiceChipPill(AppLocalizations.of(context).catChalet,
                              selected: _selectedCategory == 'Chalet',
                              onTap: () {
                            setState(() {
                              _selectedCategory =
                          ...
                          const SizedBox(width: 8),
                          ChoiceChipPill(
                              AppLocalizations.of(context).catPenthouse,
                              selected: _selectedCategory == 'Penthouse',
                              onTap: () {
                            setState(() {
                              _selectedCategory =
                                  (_selectedCategory == 'Penthouse')
                                      ? 'All'
                                      : 'Penthouse';
                            });
                            _applyCategory();
                    ...
                    StatRow(cards: [
                      StatCard(
                          ...
                          label: 'Properties',
                          onTap: () =>
                              AppNavigation.goToOwnerProperties(context)),
                      StatCard(
                          value: dashboard == null
                              ? '—'
                          ...
                          onTap: () =>
                              AppNavigation.goToOwnerBookings(context)),
                      StatCard(
                          value: dashboard?.monthlyEarnings ?? '—',
                          label: 'EGP / month',
                          onTap: () =>
                              AppNavigation.goToOwnerEarnings(context)),
                    ]),
                    const SizedBox(height: 16),
