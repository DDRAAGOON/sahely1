...
class _CompareScreenState extends State<CompareScreen> {
  Property? _propA;
  Property? _propB;
  List<Property> _candidates = const [];
  bool _candidatesLoaded = false;

  /// Rebuilds the picker sheet while it is open, e.g. when the listings
  /// arrive after it was shown.
  ...
  Future<void> _loadCandidates() async {
    try {
      final list = await sl<RenterRepository>().getAllProperties();
      if (!mounted) return;
      setState(() => _candidates = list);
    } catch (_) {
      // The picker says nothing was found; the user can go back and retry.
    } finally {
      if (mounted) {
        setState(() => _candidatesLoaded = true);
        _refreshSheet?.call(() {});
      }
    }
  }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setModalState) {
          _refreshSheet = setModalState;
          final filteredList = _candidates.where((p) {
            final nameMatch =
                p.name.toLowerCase().contains(searchQuery.toLowerCase());
            final areaMatch =
                p.area.toLowerCase().contains(searchQuery.toLowerCase());
                            ...
                            return ListTile(
                              ...
                              leading: ClipRRect(
                                ...
                                child: AppNetworkImage(
                                    url: p.image, width: 60, height: 60),
                              ),
                              title: Row(
                                children: [
                                  Expanded(
                          ...
                          children: [
                            ...
                            WhiteCard(
                              ...
                              child: Column(children: [
                                ...
                                _divider(),
                                _tableRow('Location', _propA?.area ?? '-',
                                    _propB?.area ?? '-', 1),
                                _divider(),
                                _tableRow('Type', _propA?.type ?? '-',
                                    _propB?.type ?? '-', 2),
                                _divider(),
                                _tableRow(
                                    'Price',
                                ...
                                _tableRow(
                                    'Rating',
                                    _propA != null
                                        ? '★ ${_propA!.rating} (${_propA!.reviews})'
                                        : '-',
                                    _propB != null
                                        ? '★ ${_propB!.rating} (${_propB!.reviews})'
                                        : '-',
                                    1),
                                _divider(),
                                _tableRow(
                                ...
                                _tableRow(
                                    'Guests',
                                    _propA != null
                                        ? '${_propA!.guests} guests'
                                        : '-',
                                    _propB != null
                                        ? '${_propB!.guests} guests'
                                        : '-',
                                    1),
                                _divider(),
                                _tableRow(
                                    'Beach',
                                    _propA != null
                                        ? (_propA!.minutesToBeach != null
                                            ? '${_propA!.minutesToBeach} min'
                                            : '-')
                                        : '-',
                                    _propB != null
                                        ? (_propB!.minutesToBeach != null
                                            ? '${_propB!.minutesToBeach} min'
                                            : '-')
                                        : '-',
                                    1),
                                _divider(),
    ...
    return Padding(
      ...
      child: Row(children: [
        Expanded(
            flex: 3,
            child: Text(label,
                style: AppTheme.dm(size: 12, color: AppColors.textSecondary))),
        Expanded(
            flex: 2,
            child: Center(
                child: Text(val1,
                    style: AppTheme.dm(
                        size: 12,
                        weight:
                            highlight == 1 ? FontWeight.w700 : FontWeight.w400,
                        color: highlight == 1
                            ? AppColors.gold
                            : AppColors.navy)))),
        Expanded(
            ...
            child: Center(
                child: Text(val2,
                    style: AppTheme.dm(
                        size: 12,
                        weight:
                            highlight == 2 ? FontWeight.w700 : FontWeight.w400,
                        color: highlight == 2
                            ? AppColors.gold
                            : AppColors.navy)))),
      ]),
