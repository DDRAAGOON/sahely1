  ...
  Widget build(BuildContext context) {
    return PhoneScaffold(
      child: Stack(children: [
        const _BrokerProfileContent(),
        FloatingNav(active: 4, items: FloatingNav.brokerTabs, onTap: (i, _) => brokerNav(context, i)),
      ]),
    );
  }
}

class _BrokerProfileContent extends StatelessWidget {
  const _BrokerProfileContent();
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        _brokerCta(context, 'My Role Dashboard', 'Tier · Commissions · Portfolio', Icons.workspace_premium_outlined, '/broker/dashboard', const [Color(0xFF2F8F86), Color(0xFF1D5E57)]),
        const SizedBox(height: 12),
        GestureDetector(
        ...
        const SizedBox(height: 12),
        _brokerCta(context, 'AL MAWSEM Season Pass', 'Wave Rider · 18 ★ · earn stars when you rent', Icons.waves, '/mawsem', const [Color(0xFF2A2418), AppColors.navy]),
        const SizedBox(height: 12),
  ...
  Widget _brokerCta(BuildContext context, String title, String sub, IconData icon, String route, List<Color> colors) => GestureDetector(
